...
CUSTOM_HTTP_PORT=***  # DO NOT USE 5000 as it creates a duplicate server config block in Nginx and pukes
CUSTOM_HTTPS_PORT=*** # DO NOT USE 5001 as it creates a duplicate server config block in Nginx and pukes
...
if [ "$BACKUP_FILES" == "true" ]; then
  ...
  cp -r /***/syno/share/nginx/ "$CURRENT_BACKUP_DIR"
fi
...
sed -i "s/^\([ \t]\+listen[ \t]\+[]:[]*\)$HTTP_PORT_TO_REPLACE\([^0-9]\)/\1$HTTP_NEW_PORT\2/" /***/syno/share/nginx/*.mustache
sed -i "s/^\([ \t]\+listen[ \t]\+[]:[]*\)$HTTPS_PORT_TO_REPLACE\([^0-9]\)/\1$HTTPS_NEW_PORT\2/" /***/syno/share/nginx/*.mustache

echo "Made these changes:"
diff /***/syno/share/nginx/ $CURRENT_BACKUP_DIR 2>&1 | tee $CURRENT_BACKUP_DIR/changes.log
echo " "
